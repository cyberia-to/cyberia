---
tags: cyberia, research
crystal-type: entity
crystal-domain: cyberia
---
# research

working research packs for [[cyberia]] — each folder is one project with a `README.md` index.

| project | what |
|---------|------|
| [[genome-protocol]] | attested genome / kinship proofs without a central registry |
| [[migration-market]] | status currencies, funnel, and nomad awareness model |
| [[cyber-sheep]] | autonomous energy platform on a biomechanical chassis |
| [[cyb-land]] | cyb.land visitor product — visit, stay, activities, attractions |
| [[oxytocin]] | Sayan wellness space — thermal circuit, membership, economics |
| [[mimi]] | LLM assistant research notes (code stays in mimi repo) |
